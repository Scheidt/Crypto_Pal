...
In repeating-key XOR, you'll sequentially apply each byte of the key; the first byte of plaintext will be XOR'd against I, the next C, the next E,
...
func encryptRepeatingXor(msg, key []byte) []byte{
	xored := make([]byte, len(msg))
	for i := range msg {
		xored[i] = msg[i] ^ key[i%len(key)]
	}
	return xored
}
